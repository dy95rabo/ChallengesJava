...
public class RecursionReversedListIndex {
	...
	public static Object[] getItemsAt(Object[] arr, String par) {
		int startPos = arr.length % 2 == (par.equals("even") ? 0 : 1) ? 0 : 1;
		LinkedList<Object> tmp = new LinkedList<>();
		recursiveAddElement(arr, tmp, startPos);
		return tmp.toArray();
	}

	private static void recursiveAddElement(Object[] arr, LinkedList<Object> tmp, int nextElementIndex) {
		if (nextElementIndex >= arr.length) {
			return;
		}
		tmp.add(arr[nextElementIndex]);
		recursiveAddElement(arr, tmp, nextElementIndex + 2);
	}
	// public static Object[] getItemsAt(Object[] arr, String par) {
	// int startPos = arr.length%2==(par.equals("even")?0:1)?0:1;
	// LinkedList<Object> tmp = new LinkedList<>();
	// for (int i = startPos; i < arr.length; i+=2) {
	// tmp.add(arr[i]);
	// }
	// return tmp.toArray();
	// }
	...
	public static void main(String[] args) {
		getItemsAt(new Object[]{"E","D","A","B","I","T"}, "even");
		getItemsAt(new Object[]{"E","D","A","B","I","T"}, "odd");
	}
